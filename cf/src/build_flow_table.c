    ...
    for (r = 0; r < maxr; r++) {
        for (c = 0; c < maxc; c++) {
            ...
            if ((patch[inx] > 0) && (zone[inx] > 0) && (hill[inx] > 0)) {
            	PatchKey_t k = { patch[inx], zone[inx], hill[inx] }; ///<<<------ k is an array
                
                //<<---------- below is building the "patchHashTable"
            	pch = patchHashTableGet(patchTable, k);
            	if ( PATCH_HASH_TABLE_EMPTY == pch ) {
            		num_patches++;
            		pch = num_patches;//scanning order/index, going to patchTable->entry->value (can be overwritten)
            		patchHashTableInsert(patchTable, k, pch);
            	}

                flow_table[pch].patchID = patch[inx]; //pch is scan order of patch(not grid)
                ...
                if ( (STREAM_CONNECTIVITY_RANDOM == sc_flag) || (SLOPE_STANDARD != slp_flag)) {
                ...
                if(!surface || flow_table[pch].land != LANDTYPE_ROOF) {
                    int num_adj =  check_neighbours(r, c, patch, zone, hill, stream, roofs, &flow_table[pch],
                                                    ...
                                                    cell, surface);
                                    //this function "check_neighbours" find and add!!
                    if(num_adj < 0) {
                        fprintf(stderr, "ERROR: An error occurred while determing patch neighbors.\n");
                        return -1;
                    }
                    flow_table[pch].num_adjacent += num_adj; //<-- num of neighbour
