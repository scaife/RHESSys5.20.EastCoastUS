...
int check_neighbours(int inputRow, int inputCol, int *patch, int *zone, int *hill,
    ...
    int new_adj = 0;
        ...
        // flow_entry is a pointer pointing to the current "patch"
        // inputRow and inputCol is the r and c of the current "grid"
    for (r = -1; r <= 1; r++) {
        for (c = -1; c <= 1; c++) {
            /* don't look at neighbours beyond the edge */
            if ((inputRow + r < maxr) && (inputRow + r >= 0) && (inputCol + c < maxc) && (inputCol + c >= 0)) {
                ...
                p_neigh = patch[index];//it's in vector form, need "row_col_to_index" to find the vector location
                h_neigh = hill[index];
                z_neigh = zone[index];
                ...
                // flow_entry = current "patch" itself!!
                if (((p_neigh != flow_entry->patchID)
                    ...
                    if ((flow_entry->land == 1) && (stream_neigh > 0)) {
                        if ((flow_entry->num_dsa == 0)) {
                            ...
                            // creating neighbour to the current patch!!
                    ...
                    if ((num_adj == 0)) {
                        ...
                        } /* end new entry to the neighbor */
                        // if neighbor is already in the neighbor list, then do nothing yet; accumulate perimeter below!
                    }// search exisiting neighbor list
                    ...
                    if (abs(c + r) * 1.0 / 1.0 == 1.0) {
                        ...
                        // diagonal
                        flow_entry->adj_ptr->perimeter += cell * 1.0 / sqrt(2.0); // = cell *sqrt(0.5);
                    }
