...
int compute_gamma(struct flow_struct *flow_table, int num_patches, PatchTable_t *patchTable, FILE *f1,
    ...
    for (pch = 1; pch <= num_patches; pch++) {
        ...
        flow_table[pch].internal_slope = flow_table[pch].internal_slope / flow_table[pch].area;//<---
    ...
    max_ID = sort_flow_table(flow_table, num_patches, patchTable); //<<<-------- sort by elevation!! (not a real flow structure analysis)
    ...
    for (pch = 1; pch <= num_patches; pch++) {
        ...
        for (neigh = 1; neigh <= flow_table[pch].num_adjacent; neigh++) {
            ...
            aptr->gamma = (float) (aptr->perimeter * mult * aptr->slope); // mult = 1 always
            ...
            if (aptr->gamma < 0.0){
                ...
                */
                ...
                flow_table[pch].total_gamma += aptr->gamma; //<<---- (m/m) * m = m
                flow_table[pch].total_perimeter += aptr->perimeter; //<<-------- total perimeter
                flow_table[pch].slope += aptr->slope * aptr->perimeter; //<<------- weighted sum slope
            }
        ...
        if (flow_table[pch].total_gamma == 0.0)
            ...
            flow_table[pch].slope = flow_table[pch].slope
                / flow_table[pch].total_perimeter; //<<------- weighted average slope

        for (neigh = 1; neigh <= flow_table[pch].num_adjacent; neigh++) {
            if (flow_table[pch].total_gamma != 0.0)
                aptr->gamma = aptr->gamma / flow_table[pch].total_gamma; //<<------ set neighbor gamma to frac_gamma
            else
                aptr->gamma = 0.0;
            aptr = aptr->next;
        ...
        flow_table[pch].gamma_neigh = flow_table[pch].total_gamma; //<<------- gamma_neigh?  total_gamma is modified below!

        //////////////////////------------------------ mult = 1.0 always!! see LINE 105 above
        //////////////////////------------------------ flow_table[pch].area = #grids --> flow_table[pch].area * cell * cell = actual patch area in m2
        if (SLOPE_STANDARD == slp_flag) {
            // this is default!
            flow_table[pch].total_gamma = mult * flow_table[pch].slope
                * flow_table[pch].area * cell * cell; // why re-assigning?
            ;
            // patch_total_gamma = weighted average slope * patch area; (m2)
            // patch_total_gamma *= (Ksat) * (m) in rhessys initialization => (m/time)*m *m2
            // subsurface sat. flow = patch_total_gamma * transimissivity(integration part) = (m4/time)
        }
